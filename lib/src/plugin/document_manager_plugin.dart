...
class DocumentManagerPlugin {
  ...
  static Future<Uri?> openDocumentTree({Uri? initialLocation}) {
    if (Platform.isAndroid) {
      return AndroidIntentPlugin.openDocumentTree(initialLocation: initialLocation);
    } else {
  ...
  static Future<Uri> createSubTreeUri({required Uri treeUri, required String displayName}) {
    if (Platform.isAndroid) {
      return AndroidContentResolverPlugin.createSubTreeUri(treeUri: treeUri, displayName: displayName);
    } else {
      return IosUrlPlugin.createFileURL(fileURLWithPath: displayName, isDirectory: true, relativeTo: treeUri);
    }
  }

  static Future<Uri> copyFileToTreeUri({required File file, required Uri treeUri, String? displayName}) {
    if (Platform.isAndroid) {
      return AndroidContentResolverPlugin.copyFileToTreeUri(file: file, treeUri: treeUri, displayName: displayName);
    } else {
      return Future<Uri>(() async {
        ...
        await IosFileManagerPlugin.copyItem(at: Uri.file(file.path), to: fileUri);
        return fileUri;
      });
    }
  }
  ...
  static Future<List<File>> import({
    ...
    if (Platform.isAndroid) {
      ...
      for (final uri in uris) {
        ...
        await AndroidContentResolverPlugin.copyContentUriToFile(contentUri: uri, file: file);
        files.add(file);
      }
      return files;
    } else {
      ...
      for (final uri in uris) {
        ...
        try {
          ...
          await IosFileManagerPlugin.copyItem(at: uri, to: Uri.file(path));
        } finally {
          await IosUrlPlugin.stopAccessingSecurityScopedResource(uri).asyncIgnore();
        }
        files.add(File(path));
  ...
  static Future<List<Uri>> export({required List<File> files, Uri? initialLocation}) async {
    if (Platform.isAndroid) {
      final uris = <Uri>[];
      if (files.isNotEmpty) {
        final uri = await AndroidIntentPlugin.openDocumentTree(initialLocation: initialLocation);
        if (uri != null) {
          for (final file in files) {
            await AndroidContentResolverPlugin.copyFileToTreeUri(file: file, treeUri: uri).then((value) {
              uris.add(value);
            });
          }
        }
...
class DocumentType {
  ...
  static const image = DocumentType(mimeType: "image/*", utType: "public.image");
  static const video = DocumentType(mimeType: "video/*", utType: "public.movie");
  static const audio = DocumentType(mimeType: "audio/*", utType: "public.audio");

  final String mimeType;
  final String utType;

  const DocumentType({required this.mimeType, required this.utType});
